/**
 * MentionNodeView - Custom React NodeView for TipTap mentions
 *
 * Renders mentions as EntityLink components with HoverCard preview support.
 * This allows mentions in .note files to have the same hover preview
 * functionality as EntityLink components elsewhere in the app.
 */
...
export function MentionNodeView({ node }: NodeViewProps) {
  ...
  const mentionContent = (
    <span
      className={cn(
        'mention inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md font-medium text-sm cursor-pointer transition-all',
        isNotFound
  ...
  return (
    <NodeViewWrapper as="span" className="inline">
      <HoverCard openDelay={200} closeDelay={100}>
        <HoverCardTrigger asChild>{mentionContent}</HoverCardTrigger>
