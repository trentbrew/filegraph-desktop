 ...
 * AppRail Component
 *
 * Vertical icon-only rail on the far left for app switching.
 * Replaces the horizontal app tabs that were in the CommandBar dock.
 * Tooltip on hover, highlight on active, marketplace (+) at bottom.
 */
...
export function AppRail({ className }: { className?: string }) {
  ...
  const Separator = () => <div className="mx-auto w-5 h-[1.2px] bg-foreground/15" aria-hidden="true" />

  return (
    <TooltipProvider delayDuration={300}>
      <div className="bg-transparent pl-3 pb-3">
        <div
          className={cn(
            'flex flex-col items-center p-2 gap-1 w-12 shrink-0 h-full bg-card rounded-xl border',
            className,
          )}>
          {/* Home section apps */}
              ...
              return (
                <Tooltip key={id}>
                  ...
                  <TooltipContent side="right" sideOffset={8}>
                    <p>{app.name}{app.shortcut ? ` (${app.shortcut})` : ''}</p>
                  </TooltipContent>
                </Tooltip>
              )
            })}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden flex flex-col items-center gap-1 scrollbar-none">
            {visibleApps.map((id) => {
              const app = getApp(id)
              const Icon = app.icon
              const isActive = activeApp === id
