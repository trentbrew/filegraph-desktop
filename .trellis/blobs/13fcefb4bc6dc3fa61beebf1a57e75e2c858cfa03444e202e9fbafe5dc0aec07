...
export const GanttSidebarHeader: FC = () => (
  <div
    ...
    style={{ height: 'var(--gantt-header-height)' }}>
    <p className="flex-1 truncate text-left">Items</p>
...
export const GanttColumn: FC<GanttColumnProps> = ({ index, isColumnSecondary }) => {
  return (
    <div className={cn('group relative h-full overflow-hidden', isColumnSecondary?.(index) ? 'bg-muted/50' : '')} />
  )
}
  ...
  return (
    <div
      ...
      <div
        ...
        <DndContext sensors={[mouseSensor]} modifiers={[restrictToHorizontalAxis]} onDragEnd={onDragEnd}>
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  return (
    <GanttContext.Provider
      ...
      <div
        className={cn(
          'gantt relative grid h-full w-full flex-none select-none overflow-auto rounded-sm bg-muted/30',
          range,
          className,
        )}
        style={{
