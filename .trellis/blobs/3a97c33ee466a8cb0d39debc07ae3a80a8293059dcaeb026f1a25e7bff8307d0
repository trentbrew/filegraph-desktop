...
import { Range, Root, Thumb, Track, type SliderProps } from '@radix-ui/react-slider';
...
export const ColorPicker = ({
  ...
  const safeColor = (v: any, d: any) => {
    try {
      return v ? Color(v) : Color(d);
    } catch (e) {
      console.warn('ColorPicker: Invalid color value', v, e);
      return Color(d);
    }
  };

  const selectedColor = safeColor(value, defaultValue);
  const defaultColor = safeColor(defaultValue, '#000000');
...
export const ColorPickerOutput = ({
  ...
  return (
    <Select value={mode} onValueChange={setMode}>
      <SelectTrigger className="h-8 w-18 shrink-0 text-xs" {...props}>
