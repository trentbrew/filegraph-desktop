...
export interface TQLActions {
  ...
  ) => Promise<{ described: number; skipped: number; errors: number }>
}
...
export function useTQL(): [TQLState, TQLActions] {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const envAutoScan = (import.meta as any).env?.VITE_TQL_AUTO_SCAN_HOME as string | undefined
        let shouldAutoScanHome = true
        if (envAutoScan != null) {
          shouldAutoScanHome = !(envAutoScan === 'false' || envAutoScan === '0')
        }
        if (params.has('autoScanHome')) {
          shouldAutoScanHome = params.get('autoScanHome') === 'true'
        }
        ...
        if (import.meta.env.DEV) {
          ...
          // Note: caption_image command not implemented in Rust backend yet
          ;(window as any).describeImage = async (filePath: string) => {
            console.warn('[TQL Hook] caption_image command not implemented in Rust backend')
            ...
            // Use the frontend ollama library directly instead
            return await captionImageWithLlava(filePath)
          }
        ...
        if (shouldAutoScanHome) {
          void (async () => {
            try {
              const homeDir = await path.homeDir()
              const vaultDir = `${homeDir}/.filegraph`
              console.log('[TQL Hook] Auto-scanning default vault:', vaultDir)

              setState((prev) => ({ ...prev, scanning: true, error: null }))
              await runtime.initialScan(vaultDir, (progress) => {
                setState((prev) => ({
                  ...prev,
              ...
              console.error('[TQL Hook] Auto-scan failed:', err)
              setState((prev) => ({
                ...prev,
                scanning: false,
                scanProgress: null,
  ...
  const walkDir = useCallback(
    async function* (dir: string): AsyncGenerator<DirItem> {
  ...
  const describeImagesInDir = useCallback(
    async (dirPath: string, opts?: { concurrency?: number }) => {
      const runtime = runtimeRef.current
      if (!runtime) throw new Error('TQL Runtime not initialized')

      // Use reasonable concurrency (default 2, max 4) to avoid overwhelming Ollama
      const concurrency = Math.max(1, Math.min(4, opts?.concurrency ?? 2))
      let described = 0,
        skipped = 0,
        errors = 0

      // Helper to sleep between requests (only used for retry backoff)
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
          try {
            ...
            const isResourceError =
              err?.message?.includes('resource limitations') ||
              err?.message?.includes('runner process no longer running')

            if (isLastAttempt || !isResourceError) {
              throw err
            }

            // Exponential backoff with jitter
            const delay = baseDelay * Math.pow(2, attempt) + Math.random() * 1000
            console.warn(`[TQL Hook] Retry ${attempt + 1}/${maxRetries} after ${Math.round(delay)}ms:`, err?.message)
            await sleep(delay)
          }
        }
      ...
      const runTask = async (item: DirItem) => {
        try {
          ...
          // Add retry logic for caption generation
          const { description, model } = await retryWithBackoff(() => captionImageWithLlava(item.path))
          ...
          // REMOVED: Artificial 500ms delay that was throttling throughput
          // Let Ollama handle backpressure internally via concurrency control
        } catch (err) {
  ...
  const handleRuntimeEvent = useCallback((event: RuntimeEvent) => {
    switch (event.type) {
      ...
      case 'ingest_done':
        ...
        console.log(`[TQL Hook] Scan complete: ${event.totalFiles} files in ${(event.durationMs / 1000).toFixed(2)}s`)
        break
      ...
      case 'query_slow':
        console.warn(`[TQL Hook] Slow query (${event.durationMs}ms): ${event.query}`)
        break

      case 'error':
        console.error(`[TQL Hook] Error ${event.code}: ${event.message}`, event.context)
        setState((prev) => ({
          ...prev,
          error: event.message,
        }))
  ...
  const actions: TQLActions = useMemo(
    ...
    [scanDirectory, pushFSEvent, getRuntime, describeImagesInDir],
  )
