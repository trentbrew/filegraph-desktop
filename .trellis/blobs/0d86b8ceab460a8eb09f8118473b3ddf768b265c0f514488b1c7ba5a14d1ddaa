...
import { Folder, Loader2, AlertCircle, Lock, Pause, Play, MoreHorizontal, Bot, Database } from 'lucide-react'
import {
  type IndexingState,
  type StatusBarTick,
  getStateLabel,
...
export function StatusBar({ onIndexFolder, onVaultSwitch }: StatusBarProps = {}) {
  ...
  const percent = statusTick?.counts.total ? Math.round((statusTick.counts.done / statusTick.counts.total) * 100) : 0
  ...
  const getStatusText = () => {
    if (fsmState === 'idle' && stats) {
      return `Graph ready — ${stats.entityCount.toLocaleString()} files • ${stats.factCount.toLocaleString()} facts`
    }
    if (fsmState === 'paused') {
    ...
    <>
            ...
            {stats && !scanning && (
              <div className="grid grid-cols-4 gap-3 text-xs">
                <div>
                  ...
                  <div className="font-semibold">{stats.entityCount.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Facts</div>
                  <div className="font-semibold">{stats.factCount.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-muted-foreground">Links</div>
                  <div className="font-semibold">{stats.linkCount.toLocaleString()}</div>
                </div>
              </div>
            )}

            {/* Error Display */}
            {error && <div className="text-xs text-destructive bg-destructive/10 rounded px-2 py-1.5">{error}</div>}
          </div>
        </div>
      )}
      ...
      <div
        ...
        <button
          ...
          title={`Current vault: ${statusTick?.vault.path || '~'}${onVaultSwitch ? ' • Click to switch' : ''}`}
          aria-label={`Vault: ${statusTick?.vault.name || '~'}`}>
          {statusTick?.vault.locked && <Lock className="h-3.5 w-3.5 mr-1" aria-label="Locked" />}
          <Folder className="h-4 w-4" aria-hidden="true" />
          <span className="font-medium truncate max-w-[120px]">{statusTick?.vault.name || '~'}</span>
        </button>
        ...
        {stats && !scanning && (
          <>
            <div className="flex items-center gap-1.5 text-muted-foreground">
              <span className="font-medium">{stats.entityCount.toLocaleString()}</span>
              <span>files</span>
            </div>
            <Separator orientation="vertical" className="mx-2 h-5" />
          </>
        ...
        <div
          ...
          <span className={scanning ? 'font-medium text-foreground' : ''}>{getStatusText()}</span>
        </div>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          ...
          <Button
            ...
            {/* <Bot
              ...
              aria-hidden="true"
            /> */}
            {statusTick?.vault.locked && <span className="text-[10px] text-muted-foreground">Locked</span>}
          </Button>
        </div>
      </div>
    </>
